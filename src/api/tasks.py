...
@router.patch(
    "/users/{id_user}/todo_lists/{id_list}/{id_task_to_patch}",
    tags=["Задачи"],
...
async def patch_task(
    ...
    query = (
        ...
        .join(ListsORM)
        .where(
            TasksORM.id_task == id_task_to_patch,
            ListsORM.id_list == id_list,
            ListsORM.user_id == id_user)
    )
    result = await session.execute(query)
...
@router.delete(
    "/users/{id_user}/todo_lists/{id_list}/{id_task_for_delete}",
    tags=["Задачи"],
    summary="Удалить задачу по айди",
    status_code=status.HTTP_204_NO_CONTENT)
async def delete_task(
    id_task_for_delete: int,
    id_user: int,
    id_list: int,
    session: SessionDep
):
    query = (
        ...
        .join(ListsORM)
        .where(
            TasksORM.id_task == id_task_for_delete,
            ListsORM.id_list == id_list,
            ListsORM.user_id == id_user)
    )
    result = await session.execute(query)
