...
async def test_post_new_list_success(ac: AsyncClient, auth_header: dict):
    ...
    """
    payload = {"title": "Список продуктов", "description": "Продукты на праздник"}

    response = await ac.post("/me/to-do-lists", json=payload, headers=auth_header)
...
@pytest.mark.asyncio(loop_scope="session")
@pytest.mark.parametrize("empty_payload", [{"title": ""}, {"description": ""}])
async def test_post_new_list_empty_data(
    ac: AsyncClient, empty_payload, auth_header: dict
):
    """
...
async def test_post_new_list_unauthorized(ac: AsyncClient):
    ...
    """
    payload = {"title": "Список", "description": "Описание"}
...
@pytest.mark.asyncio(loop_scope="session")
async def test_get_all_lists_with_pagination(
    ac: AsyncClient, auth_header: dict, create_many_test_todo_lists
):
    """
    ...
    response = await ac.get(
        "/me/to-do-lists", headers=auth_header, params={"limit": 2, "offset": 0}
    )
    ...
    data = response.json()
    assert len(data) == 2
    assert TypeAdapter(list[ListResponseSchema]).validate_python(
        data
    )  # проверка листа схем пайдентик


@pytest.mark.parametrize("limit, offset", [(0, 0), (25, 0), ("a", "b")])
@pytest.mark.asyncio(loop_scope="session")
async def test_get_all_lists_with_incorrect_pagination_params(
    ac: AsyncClient, auth_header, limit, offset
):
    """
...
@pytest.mark.asyncio(loop_scope="session")
async def test_patch_list_success(
    ac: AsyncClient,
    auth_header,
    create_test_todo_list,
    payload,
    expected_title,
    expected_desc,
):
...
@pytest.mark.asyncio(loop_scope="session")
async def test_patch_list_incorrect_payload(
    ac: AsyncClient, auth_header, create_test_todo_list, bad_payload
):
    """
...
async def test_patch_list_not_found(ac: AsyncClient, auth_header):
    ...
    """
    response = await ac.patch(
        "/me/to-do-lists/99999", headers=auth_header, json={"title": "any_text"}
    )
...
async def test_patch_list_unauthorized(ac: AsyncClient):
    ...
    """
    response = await ac.patch("/me/to-do-lists/1", json={"title": "any_text"})
...
async def test_delete_list_success(
    ac: AsyncClient, create_test_todo_list, auth_header, session
):
    """
    ...
    response = await ac.delete(
        f"/me/to-do-lists/{existing_list_id}", headers=auth_header
    )
...
async def test_delete_list_not_found(ac: AsyncClient, auth_header):
    ...
    """
    response = await ac.delete("/me/to-do-lists/99999", headers=auth_header)
