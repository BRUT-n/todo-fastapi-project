...
async def test_add_todo_list_success(create_test_user: UsersORM):
    ...
    list_to_create = ListAddSchema(title="ListTitle", description="ListDescription")
...
async def test_get_lists_success(create_test_todo_list: ListsORM):
    ...
    assert lists_to_get is not None
...
async def test_patch_list_title_success(create_test_todo_list: ListsORM):
    ...
    patch_data = ListPatchSchema(title="RenamedTitle")
    ...
    assert patched_list.title == "RenamedTitle"
...
async def test_patch_list_all_fields_success(create_test_todo_list: ListsORM):
    existed_list = create_test_todo_list
    patch_data = ListPatchSchema(title="RenamedTitle", description="EditedDescription")
    ...
    assert patched_lits.title == "RenamedTitle"
    assert patched_lits.description == "EditedDescription"

@pytest.mark.asyncio(loop_scope="session")
async def test_patch_list_not_found(create_test_user: UsersORM):
    existed_user = create_test_user
    patch_data = ListPatchSchema(title="Title", description="Description")
