import pytest
from src.auth.dependencies import get_user_status_by_token, register_user, validate_credentials
...
async def test_validate_credentionals_invalid_email():
    with pytest.raises(exceptions.InvalidCredentialsException):
        await validate_credentials(username="invalid@test.com", password="some_password")

@pytest.mark.asyncio(loop_scope="session")
async def test_validate_credentionals_wrong_password(create_test_user):
    existed_user_email = create_test_user.email
    with pytest.raises(exceptions.InvalidCredentialsException):
        await validate_credentials(username=existed_user_email, password="wrong_password")
